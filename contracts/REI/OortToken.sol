...
contract OortToken is DelegateERC20, Ownable {
    using SafeMath for uint256;
    
    uint256 public constant maxSupply  =        1050000000e18;

    uint256 public constant investorSupply =    105000000e18;
    uint256 public constant teamSupply =        126000000e18;

    uint256 public constant chefPoolSupply =    220500000e18;
    uint256 public constant swapMiningSupply =   89250000e18;
    uint256 public constant lendPoolSupply =    136500000e18;

    uint256 public  airdropSupply =     5250000e18;
    uint256 public  opSupply      =     105000000e18;
    uint256 public  daoSupply     =     262500000e18;

    uint256 public ChefPoolMint;
    uint256 public swapPoolMint;
    uint256 public lendPoolMint;
    ...
    modifier onlySwapPool{
        require(msg.sender == swapPool,"only SwapPool");
        _;
    }

    modifier onlyLendPool{
        require(msg.sender == lendPool,"only LendPool");
        _;
    }
    ...
    function setSwapAddr(address _swapPool) public onlyOwner{
       swapPool = _swapPool;
    }

    function setlendAddr(address _lendPool) public onlyOwner{
       lendPool = _lendPool;
    }

    function ChefMint(uint256 _amount) public onlyChef{
        if(ChefPoolMint.add(_amount) >= chefPoolSupply)  {
            _amount = chefPoolSupply.sub(ChefPoolMint);
        }
        ...
        if(_amount == 0){
         ...
         ChefPoolMint = ChefPoolMint.add(_amount);
    }

    function SwapMint(uint256 _amount) public onlySwapPool{
        if(swapPoolMint.add(_amount) >= swapMiningSupply)  {
            _amount = swapMiningSupply.sub(swapPoolMint);
        }
        ...
        if(_amount == 0){
         ...
         _mint(_msgSender(),_amount);
         swapPoolMint = swapPoolMint.add(_amount);
    }

    function lendMint(uint256 _amount) public onlyLendPool{
        if(lendPoolMint.add(_amount) >= lendPoolSupply)  {
            _amount = lendPoolSupply.sub(lendPoolMint);
        }
    ...
    function OtherMint(address _dao,uint256 _amount) public onlyOwner {
        ...
        daoSupply = daoSupply.sub(_amount);
    }
